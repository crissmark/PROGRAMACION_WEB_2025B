<?php
session_start();

// Validar que el usuario esté autenticado
if (!isset($_SESSION['usuario'])) {
    header("Location: index.php?error=noauth");
    exit;
}

// Lógica del contador de sesión
if (isset($_SESSION['contador_sesion'])) {
    $_SESSION['contador_sesion']++;
} else {
    $_SESSION['contador_sesion'] = 1;
}
$visitas = $_SESSION['contador_sesion'];
$color = $_SESSION['color'];
?>
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Contador de Sesión</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body style="color: <?php echo htmlspecialchars($color); ?>;">
    <div class="container mt-5">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card shadow-sm text-center">
                    <div class="card-body" style="color: #212529;">
                        <h2 style="color: <?php echo htmlspecialchars($color); ?>;">
                            Hola, <?php echo htmlspecialchars($_SESSION['usuario']); ?>
                        </h2>
                        <h1 class="display-4">Contador (SESIONES)</h1>
                        <p class="lead">Has visitado esta página:</p>
                        <h2 class="display-1"><?php echo $visitas; ?></h2>
                        <p class="text-muted">Este contador se reiniciará cuando cierres el navegador o la sesión.</p>
                        
                        <hr>
                        <a href="bienvenida.php" class="btn btn-secondary">&laquo; Volver</a>
                        <a href="logout.php" class="btn btn-danger">Cerrar Sesión</a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</body>
</html>